...
def _run_fold_diagnostics(model, translator, survival_head, val_loader, device, cfg):
    ...
    Post-training diagnostics printed for fold 0:
      ...
      - Per-direction cosine similarity
    ...
    for src in range(3):
        for tgt in range(3):
            ...
            if cos_vals:
                ...
                print(f"  {dir_names[src]:6s} -> {dir_names[tgt]:6s}: cosine={avg_cos:.4f}")
...
def run_cross_validation(cfg, device):
    ...
    Fold-0 diagnostics (cosine, PoE dominance) are printed after training.
