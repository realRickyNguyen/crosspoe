...
def run_mcar_crossPoe(fold_results, cfg, device,
                      missing_rates=None, modalities=None):
    """
    Run a full MCAR grid for CrossPoE (with CrossModalTranslator).
    ...
    Args:
        ...
        cfg           : config dict (must include n_folds, seed, n_latent,
                        translation_hidden_dim, alpha, batch_size, n_epochs,
    ...
    _print_mcar_summary(results, modalities, missing_rates)
    return results


def run_mcar_single(fold_results, cfg, device,
                    missing_rates=None, modalities=None):
    """
    Run a full MCAR grid for a baseline model without CrossModalTranslator.

    Suitable for CrossAE, PoE-VAE, MoE-VAE and other baselines whose
    forward() accepts only a batch dict and returns z_surv.

    Args:
        fold_results  : list of fold dicts returned by the baseline's training
                        function; must contain model_state["model"] and
                        model_state["survival"]
        cfg           : config dict (must include n_folds, seed, n_latent,
                        batch_size, n_epochs)
        device        : torch.device
        missing_rates : floats in [0, 1]; default [0.0, 0.3, 0.5, 0.7, 0.9]
                ...
                with torch.no_grad():
                    for batch in loader:
                        ...
                        if valid.any():
                            ...
                            all_pfi_time.append(batch["pfi_time"][valid].cpu())
                            all_pfi_event.append(batch["pfi_event"][valid].cpu())
...
def _print_mcar_summary(results, modalities, missing_rates):
    print(f"\n{'='*50}")
